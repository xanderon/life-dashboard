    ...
    <section className="surface-card surface-card--soft p-4 sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
        {links.map((link) => (
          <a
            key={link.href}
            className="group flex min-w-0 items-center gap-3 rounded-[1.15rem] border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--panel-2)_82%,transparent)] px-3 py-3 transition-colors duration-150 hover:border-[var(--border-strong)] hover:bg-[color:color-mix(in_srgb,var(--accent-soft)_56%,var(--panel-2))]"
            href={link.href}
            target="_blank"
            rel="noreferrer"
          >
            <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--surface-highlight)_85%,transparent)] text-[var(--accent)]">
              {link.icon}
            </span>
